...
PROMPT="${CMESH_PROMPT:-Run a local CDIP tensor envelope smoke.}"
OUT_DIR="${CMESH_CDIP_TENSOR_SMOKE_DIR:-/tmp/cmesh-cdip-tensor-envelope-smoke-$(date -u +%Y%m%d%H%M%S)}"
    ...
    -d "$(jq -n --arg name "$name" --arg model "$MODEL_ID" '{
      ...
      resources: {
        ...
        models: [{
          ...
          family: "qwen",
          runtime: "llama.cpp",
          path: ("/tmp/cmesh-tensor-smoke/" + $model + ".gguf"),
          bytes: 5000000000,
          ready: true
        }],
        runtimes: [{
          name: "llama.cpp",
          ready: true,
          version: "logical-stage-tensor-smoke",
          ...
          stage_runtimes: [{
            ...
            ready: true,
            cli_ready: true,
            required_hooks: ["mock tensor envelope relay"]
          }]
        }]
      }
    }')" \
...
main() {
  ...
  join_worker "cdip-tensor-stage-a" | tee "$OUT_DIR/join-a.json" >/dev/null
  join_worker "cdip-tensor-stage-b" | tee "$OUT_DIR/join-b.json" >/dev/null
  ...
  echo "Preparing logical tensor stages..."
  ...
  stage1_id="$(jq -r '.stage_jobs | sort_by(.cdip_stage_index) | .[1].id' "$OUT_DIR/distributed-generate.json")"
  stage0_node="$(jq -r '.stage_jobs | sort_by(.cdip_stage_index) | .[0].assigned_to' "$OUT_DIR/distributed-generate.json")"
  stage1_node="$(jq -r '.stage_jobs | sort_by(.cdip_stage_index) | .[1].assigned_to' "$OUT_DIR/distributed-generate.json")"

  printf 'cdip-tensor-envelope-v1:%s:%s:%s' "$parent_id" "$stage0_id" "$stage1_id" > "$OUT_DIR/tensor.bin"
  byte_count="$(wc -c < "$OUT_DIR/tensor.bin" | awk '{print $1}')"
  checksum="sha256:$(shasum -a 256 "$OUT_DIR/tensor.bin" | awk '{print $1}')"

  echo "Sending deterministic tensor envelope stage0 -> stage1..."
  go run ./cmd/cmesh stage-runner decode \
    --parent-job "$parent_id" \
    --stage-job "$stage0_id" \
    --stage-index 0 \
    --step 1 \
    --payload-file "$OUT_DIR/tensor.bin" \
    --encoding "raw" \
    --dtype "f16" \
    --shape "1,4,8" \
    --checksum "$checksum" \
    --upstream-stage "$stage0_id" \
    --downstream-stage "$stage1_id" \
    --downstream-node "$stage1_node" \
    --manager "$MANAGER_URL" \
    --node-id "$stage0_node" \
    | tee "$OUT_DIR/stage-runner-decode.json" >/dev/null

  echo "Receiving and validating tensor envelope on stage1..."
  go run ./cmd/cmesh stage-runner receive \
    --parent-job "$parent_id" \
    --stage-job "$stage0_id" \
    --stage-index 1 \
    --upstream-stage "$stage0_id" \
    ...
    --shape "1,4,8" \
    --checksum "$checksum" \
    | tee "$OUT_DIR/stage-runner-receive.json" >/dev/null

  jq -e --arg checksum "$checksum" --argjson bytes "$byte_count" '
    .tensor_envelope.protocol == "cdip.tensor-envelope-v1" and
    .tensor_envelope.dtype == "f16" and
    .tensor_envelope.shape == [1,4,8] and
    .tensor_envelope.byte_count == $bytes and
    .tensor_envelope.checksum == $checksum and
    .tensor_envelope.downstream_stage_job_id != ""
  ' "$OUT_DIR/stage-runner-decode.json" >/dev/null
  ...
  jq -e --arg checksum "$checksum" --argjson bytes "$byte_count" '
    ...
    .tensor_envelope.checksum == $checksum and
    .tensor_envelope.downstream_stage_job_id != ""
  ' "$OUT_DIR/stage-runner-receive.json" >/dev/null

  post_json "/v1/cdip/jobs/$parent_id/decode" '{"step":1}' | tee "$OUT_DIR/decode.json" >/dev/null
  post_json "/v1/cdip/jobs/$parent_id/complete" '{"output":"cdip tensor envelope smoke completed"}' | tee "$OUT_DIR/complete.json" >/dev/null
  ...
  jq 'input as $decode | input as $receive | {parent_job:.parent_job.id,status:.parent_job.status,stage_count:(.stage_jobs|length),decode_tensor_envelope:$decode.tensor_envelope,receive_tensor_envelope:$receive.tensor_envelope,relay:{dtype:$receive.activation_frame.dtype,shape:$receive.activation_frame.shape,bytes:$receive.activation_frame.payload_bytes,checksum:$receive.activation_frame.checksum}}' \
    "$OUT_DIR/complete.json" "$OUT_DIR/stage-runner-decode.json" "$OUT_DIR/stage-runner-receive.json" | tee "$OUT_DIR/summary.json" >/dev/null
  echo "PASS: CDIP tensor envelope smoke succeeded"
